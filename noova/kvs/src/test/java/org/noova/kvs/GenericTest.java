...
public class GenericTest {
  ...
  protected void startTest(String testCode, String testName, int maxPointsArg) {
...
//      System.err.println("*** TIMEOUT *** ("+timeoutMillis+" ms)");
  ...
  protected synchronized void testSucceeded() {
  ...
  protected synchronized void testFailed(String explanation) throws Exception {
  ...
  synchronized void testFailed(String explanation, boolean throwException) throws Exception {
    if (!currentTestFailed) {
      numTestsFailed ++;
      timeoutThread.interrupt();
      while (true) { try { timeoutThread.join(); break; } catch (InterruptedException ie) {} }
...
System.out.println("\n"+explanation);
  ...
  protected Response readAndCheckResponse(Socket s, String which) throws Exception  {
  ...
  protected Socket openSocket(int port) throws Exception {
    Socket s = null;
    try {
