...
public class HW5SelfTest extends GenericTest {
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("delete")) try {
      ...
      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      out.print("PUT /delete/pt-mblfffz HTTP/1.1\r\nHost: localhost:8001\r\n\r\n");
      out.flush();
      r = readAndCheckResponse(s, "response");
      if (r.statusCode != 404)
        testFailed("We were trying to delete a nonexistent table, but the worker returned a " + r.statusCode + " response, when it should have returned a 404 Not Found. Here is what was in the body:\n\n" + dump(r.body));
    ...
    if (tests.contains("readrow")) try {
      ...
      String nonExistentRow = randomAlphaNum(4, 5);
        s = openSocket(8001);
        out = new PrintWriter(s.getOutputStream());
        out.print("GET /data/" + thetable + "/" + nonExistentRow + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
        out.flush();
        r = readAndCheckResponse(s, "response");
        if (r.statusCode != 404)
          testFailed("The worker returned a " + r.statusCode + " response for our GET /data/" + thetable + "/" + nonExistentRow + ", but we were expecting a 404 Not Found. Here is what was in the body:\n\n" + dump(r.body));
      ...
      startTest("pt-readrow", "Whole-row read", 5);

      thetable = "pt-" + randomAlphaNum(4, 6);
      row = randomAlphaNum(4, 5);
      col1 = randomAlphaNum(3, 5);
      col2 = randomAlphaNum(3, 5);
      val1 = randomAlphaNum(3, 5);
      val2 = randomAlphaNum(3, 5);
      ...
      if (r.statusCode != 200)
        testFailed("The worker returned a " + r.statusCode + " response for our GET /data/" + thetable + "/" + row + ", but we were expecting a 200 OK. Here is what was in the body:\n\n" + dump(r.body));
      option1 = row + " " + col1 + " " + val1.length() + " " + val1 + " " + col2 + " " + val2.length() + " " + val2 + " ";
      option2 = row + " " + col2 + " " + val2.length() + " " + val2 + " " + col1 + " " + val1.length() + " " + val1 + " ";
      if (!r.body().equals(option1) && !r.body().equals(option2))
        testFailed("In the response to our whole-row GET /" + thetable + "/" + row + ", we expected to see one of the following:\n\n  * " + option1 + "\n  * " + option2 + "\n\nbut we didn't. Here is what was in the body instead:\n\n" + dump(r.body));
    ...
    if (tests.contains("rstream")) try {{
      ...
      String nonExistentTable = randomAlphaNum(4, 6);
        s = openSocket(8001);
        out = new PrintWriter(s.getOutputStream());
        out.print("GET /data/" + nonExistentTable + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
        out.flush();
        r = readAndCheckResponse(s, "response");
        if (r.statusCode != 404)
          testFailed("The worker returned a " + r.statusCode + " response for our GET /data/" + nonExistentTable + ", but we were expecting a 404 Not Found. Here is what was in the body:\n\n" + dump(r.body));


      testSucceeded();}
      {
        startTest("pt-rstream", "Streaming read", 10);
    ...
    if (tests.contains("rename")) try {
      ...
      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      String nonexisttable = randomAlphaNum(4, 6);
      req = "PUT /rename/" + nonexisttable;
      out.print(req + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + nonexisttable.length() + "\r\n\r\n" + nonexisttable);
      out.flush();
      r = readAndCheckResponse(s, "response");
      if (r.statusCode != 404)
        testFailed("The server returned a " + r.statusCode + " response to our " + req + ", but we were expecting a 404. Here is what was in the body:\n\n" + dump(r.body));


      // test 400 pt to transit

      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      String nonpttable = "pt-" + randomAlphaNum(4, 6);
      req = "PUT /rename/" + thetable;
      out.print(req + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + nonpttable.length() + "\r\n\r\n" + nonpttable);
      out.flush();
      r = readAndCheckResponse(s, "response");
      if (r.statusCode != 400)
        testFailed("The server returned a " + r.statusCode + " response to our " + req + ", but we were expecting a 400. Here is what was in the body:\n\n" + dump(r.body));
      s.close();
      ...
      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      String existtable = randomAlphaNum(4, 6);

      req = "PUT /data/" + existtable + "/" + therow + "/" + thecol;
      out.print(req + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + thedata.length() + "\r\n\r\n" + thedata);
      out.flush();
      r = readAndCheckResponse(s, "response");
      if (r.statusCode != 200)
      ...
      req = "PUT /rename/" + thetable;
      out.print(req + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + existtable.length() + "\r\n\r\n" + existtable);
      out.flush();
      r = readAndCheckResponse(s, "response");
      if (r.statusCode != 409)
        testFailed("The server returned a " + r.statusCode + " response to our " + req + ", but we were expecting a 409. Here is what was in the body:\n\n" + dump(r.body));
      s.close();
    ...
    if (tests.contains("count")) try {{
      ...
      String nonexisttable = randomAlphaNum(4, 6);
        out.print("GET /count/" + nonexisttable + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
        out.flush();
        r = readAndCheckResponse(s, "response");
        if (r.statusCode != 404)
          testFailed("The worker returned a " + r.statusCode + " response for our GET /count/" + nonexisttable + ", but we were expecting a 404. Here is what was in the body:\n\n" + dump(r.body));
      ...
      {
        startTest("pt-count", "Row count", 5);
    ...
    if (tests.contains("pages")) try {{
      ...
      testSucceeded();}
      {
        startTest("pt-pages", "Paginated user interface", 5);
        String thetable = "pt-" + randomAlphaNum(4, 6);
        int num = 50 + (new Random()).nextInt(100);
        for (int i = 0; i < num; i++) {
          Socket s = openSocket(8001);
    ...
    if (tests.contains("pt-rename")) try {
      startTest("pt-rename", "Rename a table", 5);
      Socket s = openSocket(8001);
      PrintWriter out = new PrintWriter(s.getOutputStream());
      String thetable = "pt-" + randomAlphaNum(4, 6);
      String therow = randomAlphaNum(4, 5);
      ...
      // test 404 not exist
        ...
        s = openSocket(8001);
        out = new PrintWriter(s.getOutputStream());
        String nonexisttable = "pt-" + randomAlphaNum(4, 6);
        req = "PUT /rename/" + nonexisttable;
        out.print(req + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + nonexisttable.length() + "\r\n\r\n" + nonexisttable);
        out.flush();
        r = readAndCheckResponse(s, "response");
      ...
      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      String nonpttable = randomAlphaNum(4, 6);
      req = "PUT /rename/" + thetable;
      out.print(req + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + nonpttable.length() + "\r\n\r\n" + nonpttable);
      out.flush();
      r = readAndCheckResponse(s, "response");
      ...
      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      String existtable = "pt-" + randomAlphaNum(4, 6);
      ...
      s = openSocket(8001);
      out = new PrintWriter(s.getOutputStream());
      String newtable = "pt-" + randomAlphaNum(4, 6);
      req = "PUT /rename/" + thetable;
      out.print(req + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + newtable.length() + "\r\n\r\n" + newtable);
      out.flush();
      r = readAndCheckResponse(s, "response");
