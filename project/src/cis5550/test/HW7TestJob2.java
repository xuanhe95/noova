...
public class HW7TestJob2{
    ...
    if (args[0].equals("count")) {
		...
		for (int i=1; i<args.length; i++)
			list.add(args[i]);
		List<String> out = ctx.parallelize(list).filter(s -> Integer.valueOf(s) % 2 == 0).collect();
		Collections.sort(out);
		String ret = "";
		for (String s : out) 
			ret = ret + (ret.equals("") ? "" : ",") + s;
        ...
        List<String> out = ctx.parallelize(list).mapPartitions(iterator -> {
        ...
        for (int i = 3; i < 3 + Integer.valueOf(args[1]); i++) {
            list1.add(args[i]);
        }

        for (int i = 3 + Integer.valueOf(args[1]); i < 3 + Integer.valueOf(args[1]) + Integer.valueOf(args[2]); i++) {
            list2.add(args[i]);
        }
